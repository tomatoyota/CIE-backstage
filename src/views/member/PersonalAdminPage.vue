...
export default {
  ...
  mounted() {
    ...
    this.initializePage();
    const personalStore = usePersonalStore();
  console.log('Retrieved Search Condition:', personalStore.searchCondition);
  console.log('Retrieved Table Items:', personalStore.tableItems);
  console.log('Retrieved Paginations:', personalStore.paginations);
  ...
  methods: {
    async searchMemberList(){
      ...
      try{
        ...
        const page = this.paginations.currentPage || 1
    ...
    // console.log('Condition to store:', condition);
        // personalStore.setSearchCondition(this.searchCondition);

        await personalMemSrv.searchMemberList(frontUserId, name, level, paymentStatus,page, size, sortField, sortOrder).then((res) => {
          // api 回傳成功
          if (res.isSuccess) {
            // rtnCode 為 0000 的情況
            if(res.data.rtnCode === '0000'){
              this.tableItems = res.data.data
              this.paginations.currentPage = res.data.pagination.page
    ...
    async getAcademicsDetail(id){
      this.loading = true
      try{
        await personalMemSrv.getAcademicsDetail(id).then((res) => {
          // api 回傳成功
          if (res.isSuccess) {
            // rtnCode 為 0000 的情況
            if(res.data.rtnCode === '0000'){
    ...
    async deleteAcademicsItem(id){
      this.loading = true
      try{
        await personalMemSrv.deleteMember(id).then((res) => {
          // api 回傳成功
          if (res.isSuccess) {
            // rtnCode 為 0000 的情況
            if(res.data.rtnCode === '0000'){
    ...
    async getPaymentHistory(item){
      ...
      this.loading = true
      try{
        await personalMemSrv.getPaymentHistory(id).then((res) => {
          // api 回傳成功
          if (res.isSuccess) {
            // rtnCode 為 0000 的情況
            if(res.data.rtnCode === '0000'){
    ...
    setPage(page) {
      ...
      this.searchMemberList();
      // const personalStore = usePersonalStore();
      // personalStore.updatePagination(this.paginations);
