<template>
    <v-card>
        <v-card-title class="bg-dialog-title py-4">
            <h5 class="font-weight-bold text-h3 mb-0">審核通過確認</h5>
        </v-card-title>
        <v-card-text class="scroll-y">
            <v-table>
                ...
                <tbody>
                    <tr v-for="(word, index) in props.actionType === '多筆通過' ? selectedList : selectedWord" :key="word">
...
const passWords = () => {
    ...
    const submitWords = words.map((item) => item.editWordId);
    reviewdatabaseSrv.passReviewWords(submitWords).then((res) => {
        if (!res.isSuccess) {
            Swal.fire({
                toast: true,
                position: 'center',
